            ...
            auto sceneHandle = pViewPort->CreateScene<PlatformSDL2::DefaultRender>(
                                   ...
                                   const Platform::SceneConfig& in_rSceneConfig)
                {
                    ...
                    pRender->Draw2DText(in_rSceneConfig, spPlatformModule->Font().get(),
                                        screenLeftTopPos, Core::Math::EAnchor::EAnchor_Left,
                                   ...
                                   const Platform::SceneConfig& in_rSceneConfig)
                {
                    ...
                    auto pRender = HE_SHADER_PTR_CAST(PlatformSDL2::DefaultRender, in_spRender);
                    {
                        ...
                        auto screenLeftTopPos =
                            Core::Math::Vector2(-in_rSceneConfig.GetWidthHalf(),
                                                in_rSceneConfig.GetHeightHalf());
                        // ウィンドウが閉じたら終了
                        Core::Math::RC::Rect2D rect(screenLeftTopPos._fX, screenLeftTopPos._fY,
                                                    32.f, 32.0f);
                        // 左隅にぴったり表示しているかのテスト
                        {
                            pRender->Draw2DQuad(in_rSceneConfig, rect, Core::Math::EAnchor_Left,
                                                Core::Math::RGB::Red);
                        }

                        // 左隅の矩形と1ドット右にずらして矩形表示しているかテスト
                        {
                            rect.Set(screenLeftTopPos._fX + 33.f, screenLeftTopPos._fY, 32.0f,
                                     32.0f);
                            pRender->Draw2DQuad(in_rSceneConfig, rect, Core::Math::EAnchor_Left,
                        ...
                        {
                            rect.Set(screenLeftTopPos._fX, screenLeftTopPos._fY - 33.0f, 32.0f,
                                     32.0f);
                            pRender->Draw2DQuad(in_rSceneConfig, rect, Core::Math::EAnchor_Left,
                                                Core::Math::RGB::Yellow);
                        }
                        ...
                        // 画面真ん中に矩形表示のアンカーが真ん中
                        {
                            ...
                            pRender->Draw2DQuad(in_rSceneConfig, rect, Core::Math::EAnchor_Center,
                                                Core::Math::RGB::Red);
                        }
                        ...
                        // アンカーが左上で円描画の画面真ん中上のテスト
                        {
                            ...
                            pRender->Draw2DCircle(in_rSceneConfig, pos, Core::Math::EAnchor_Left,
                        ...
                        {
                            Core::Math::Vector2 pos(.0f, .0f);
                            pRender->Draw2DCircle(in_rSceneConfig, pos, Core::Math::EAnchor_Center,
                                                  32, Core::Math::RGB::White);
                        }
                        ...
                        // アンカー中心三角形描画テスト
                        {
                            ...
                            pRender->Draw2DTriangle(in_rSceneConfig, pos,
                                                    Core::Math::EAnchor_Center, 0.0f, 32.0f,
                                                    Core::Math::RGB::Blue);
                        }
                        ...
                        // アンカー左上の三角形描画テスト
                        {
                            ...
                            pRender->Draw2DTriangle(in_rSceneConfig, pos, Core::Math::EAnchor_Left,
        ...
        // ウィンドウ追加
        {
            ...
            auto sceneHandle = pViewPort->CreateScene<PlatformSDL2::DefaultRender>(
                       ...
                       const Platform::SceneConfig& in_rSceneConfig)
                {
                    ...
                    auto pRender = HE_SHADER_PTR_CAST(PlatformSDL2::DefaultRender, in_spRender);
                    // 点群表示
                    pRender->Draw2DPartical(in_rSceneConfig, particleObjHandle, pos);
                    // 点群が上に移動
                    pos._fY += 0.5f;
                });
            {
