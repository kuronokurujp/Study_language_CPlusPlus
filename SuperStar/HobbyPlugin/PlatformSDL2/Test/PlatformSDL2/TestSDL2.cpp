...
{
    TEST(HobbyPlugin_PlatformSDL2, FontLoad)
    {
        HE::Uint32 uStep = 0;
        ...
        auto spPlatformModule = moduleManager.Get<Platform::PlatformModule>();
        {
            ...
            auto bRet = spPlatformModule->Font()->VLoad(Platform::EFontSize_64,
                                                        HE_STR_TEXT("Resources/Font/TestFont.ttf"));
            EXPECT_EQ(bRet, TRUE);
        }
    ...
    PlatformSDL2::PlatformSDL2Module, Render::RenderModule>(
            [&uStep, &sceneHandle, &fontMatHandle]()
            {
                auto spPlatformModule = HE_ENGINE.PlatformModule();
                ...
                if (uStep == 0)
                {
                    ...
                    // フォントデータのバイナリアセットを作成
                    {
                        // ロードするフォントファイルパスを渡す
                        // シェーダーファイルも渡す.(プラットフォームによっては使わない)
                        auto bRet = spPlatformModule->VFont()->VLoad(Platform::EFontSize_64,
                ...
                else if (uStep == 2)
                {
                    ...
                    Render::Command2DTextDraw(sceneHandle, Core::Math::Vector2(0.0f, 32.0f * 3.f),
    s1, 32, Core::Math::RGB::White, Core::Math::Rect2::EAnchor::EAnchor_Left);

                    //  改行なしチェック
                    Core::Common::FixedString1024 s2(HE_STR_TEXT("Ss"));
                    Render::Command2DTextDraw(sceneHandle, Core::Math::Vector2(0.0f, 32.0f * 5.f),
    s2, 32, Core::Math::RGB::White, Core::Math::Rect2::EAnchor::EAnchor_Left);
            ...
            [&uStep, &sceneHandle, &fontMatHandle]()
            {
                ...
                else if (uStep == 3)
                {
                    ...
                    // テキストの下地で矩形が表示しているかテスト
                    {
                        ...
                        Render::Command2DTextDraw(sceneHandle, Core::Math::Vector2(0.0f, 32.0f
    * 5.f), s2, 32, Core::Math::RGB::Blue, Core::Math::EAnchor_Left, 2.0f);

                        Core::Common::FixedString1024 s3(HE_STR_TEXT("MMM"));
                        Render::Command2DTextDraw(sceneHandle, Core::Math::Vector2(0.0f, 64.0f
    * 5.f), s3, 16, Core::Math::RGB::Blue, Core::Math::EAnchor_Left, 2.0f);
                    }
                    ...
                    {
                        rect.SetPosition(0.0f, 480.0f - 32.0f, 32.0f, 32.0f,
    Core::Math::EAnchor_Left); Render::Command2DQuadDraw(sceneHandle, rect, Core::Math::RGB::White);
                    }

                    // 画面右上に矩形表示してかつアンカーが左上
                    {
                        rect.SetPosition(640.0 - 32.0f, 0.0f, 32.0f, 32.0f,
    Core::Math::EAnchor_Left); Render::Command2DQuadDraw(sceneHandle, rect, Core::Math::RGB::White);
                    }
                    ...
                    // アンカーが中心で画面真ん中下の円描画のテスト
                    {
                        ...
                        Render::Command2DCircleDraw(sceneHandle, pos, Core::Math::EAnchor_Center,
    ...
    */
            ...
            [&uStep, &renderHandle, &fontMatHandle, &uMoveStep, &pos, &prticleHandle]()
            {
                ...
                else if (uStep == 3)
                {
                    // TODO: パーティクル作成
                    {
                        ...
                        {
                            auto x = spPlatformModule->VSystem()->VGetRandByFloat(-1.0f, 1.0f);
                            auto y = spPlatformModule->VSystem()->VGetRandByFloat(-1.0f, 1.0f);
            ...
            [&uStep, &sceneHandle, &fontMatHandle, &uMoveStep, &pos]()
            {
                ...
                if (uStep == 0)
                {
                    ...
                    // フォントデータのバイナリアセットを作成
                    {
                        // ロードするフォントファイルパスを渡す
                        auto bRet =
                            spPlatformModule->VFont()->VLoad(Platform::EFontSize_64,
                                                            HE_STR_TEXT("Font/TestFont.ttf"));
                        CHECK(bRet);
                    }
                    return FALSE;
    ...
    PlatformSDL2::PlatformSDL2Module, Render::RenderModule>(
            []()
            {
                auto spPlatformModule = HE_ENGINE.PlatformModule();
                // ゲームウィンドウを作成
                auto spScreen = spPlatformModule->VScreen();
                HE_ASSERT(spScreen);
